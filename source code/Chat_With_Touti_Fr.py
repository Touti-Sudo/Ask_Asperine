...
def chatbot(question):
    ...
    if any(word in question for word in ["qui es-tu", "présentation", "présente-toi", "que fais-tu", "comment fonctionnes-tu", "chatbot", "qui t'a créé", "programmé"]):
        return "🤖 Bonjour ! Je suis ChatWithTouti, une IA non générative (bot) créée par Anes Kada avec Python pour son projet de technologie. Je suis spécialisée dans les informations sur l'aspirine. Pose-moi une question, et je ferai de mon mieux pour te répondre ! 😊"
    

    elif any(word in question for word in ["effets secondaires", "risques", "danger", "dangereuse", "allergie", "contre-indications", "saignements"]):
        info = get_aspirin_info("effets_secondaires")
        return "\n".join(info)
    

    elif any(word in question for word in ["découverte", "découvertes", "découvert"]):
        info = get_aspirin_info("decouverte")
        return info["description"]
    

    elif any(word in question for word in ["histoire", "origine", "quand", "qui a inventé", "qui a découvert"]):
        info = get_aspirin_info("histoire")
        return info["description"]
    

    elif any(word in question for word in ["ancienne", "antiquité", "traditionnelle", "histoire utilisation"]):
        info = get_aspirin_info("utilisations_anciennes")
        return "\n".join(info)
    

    elif any(word in question for word in ["utilisations modernes", "aujourd’hui", "actuelles", "antidouleur", "anti-inflammatoire", "fièvre", "anticoagulant", "AVC", "infarctus", "arthrite", "cardiologie"]):
...
@app.route("/")
def home():
    return render_template("index.html", message="Bonjour, et Bienvenue dans Chat_With_Touti ! Créé par Anes Kada pour son projet de technologie. (svp si vous êtes la prof, donnez-moi 20/20 😆)", time=int(time.time()))
